...
function App() {
  ...
  // Função para conectar carteira MetaMask conforme especificações do onboarding
  const connectWallet = async () => {
    if (!window.ethereum) {
      alert("Por favor, instale a MetaMask para interagir com o AcademiChain.");
      return;
    }
    ...
    try {
      ...
      // Solicita acesso às contas usando o método especificado eth_requestAccounts
      const accounts = await window.ethereum.request({ method: 'eth_requestAccounts' });
      
      if (accounts.length > 0) {
        setCurrentAccount(accounts[0]);
        
        // Opcional: Atualizar info de rede se conectado
  ...
  const handleSubmitReward = async (e) => {
    ...
    try {
      ...
      fetchBalance(currentAccount, provider);
    ...
    <div className="dashboard-container">
      ...
      <div className="main-workspace">
        ...
        <header className="top-navbar">
          ...
          <button 
            className={`connect-btn ${currentAccount ? 'connected' : ''}`}
            onClick={connectWallet}
            disabled={isConnecting}
          >
            <span className={`wallet-status-dot ${currentAccount ? 'green' : 'red'}`}></span>
            {currentAccount ? formatAddress(currentAccount) : "Conectar Carteira"}
          </button>
        </header>
        ...
        <main className="dashboard-content">
          ...
          <div className="stats-grid">
            <div className="stat-card balance">
              ...
              <div className="stat-value">{currentAccount ? balance : "--.--"} <span>ACAD</span></div>
            ...
            <div className="stat-card network">
              ...
              <div className="stat-value" style={{ fontSize: '1.5rem', lineHeight: '2.5rem' }}>
                {currentAccount ? network : "Desconectado"}
              </div>
